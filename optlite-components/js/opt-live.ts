...
export class OptLiveFrontend extends OptFrontend {
  ...
  // stays anchored while the student scrubs, matching the display/visualize
  // reference mode. Cleared on a new run (executeCodeFromScratch) or when the
  // user edits the code.
  persistErrorState: { html: string } | null = null;
  ...
  updateStepLabels() {
    ...
    // compile/runtime error, or stepping onto a runtime exception), we must NOT
    ...
    if (curEntry.event === 'exception' ||
      ...
      if (curEntry.advisory) {
        ...
        // not compiler/runtime failures: shown as-is.
      ...
      // Reset at the start of the next run (executeCodeFromScratch) or when
      ...
      // Non-exception step: keep the run's error anchored on screen so
      // scrubbing through the execution doesn't make it vanish — matching
      // the display/visualize reference mode.
  ...
  executeCodeFromScratch() {
    ...
    // re-appears only if THIS run errors.
    this.persistErrorState = null;
