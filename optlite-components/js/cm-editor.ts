// ============================================================================
// cm-editor.ts — CodeMirror 6 wrapper for the OPT editor pane.
//
...
//
// Exposes the small surface the existing opt-frontend.ts / opt-live.ts code
// depends on: value get/set, mode (c_cpp/python), change events, the red
// full-line "errorLine" highlight, the per-step gutter arrows (current=red,
// prev=green, overlap=both), and focus.
//
// Mobile-friendliness: 16px base font (avoids iOS auto-zoom-on-focus), line
// numbers on, soft tabs (4 spaces).
// ============================================================================
...
export interface OptCmEditorOptions {
  container: HTMLElement;      // #codeInputPane
  value: string;
  mode: "c_cpp" | "python";
  tabSize?: number;
...
export class OptCmEditor {
  ...
  constructor(opts: OptCmEditorOptions) {
    ...
    const baseTheme = EditorView.theme({
      "&": { height: "100%", fontSize: "16px" },   // >=16px avoids iOS zoom
      ".cm-scroller": {
      ...
      // ARROW IMAGE is set in css/opt-frontend.css (url() must be rewritten by
      // css-loader, which only fixes URLs in real .css files, not this JS
      // theme string). Here we only size the column and right-align the arrow.
    ...
    const startState = EditorState.create({
      ...
      extensions: [
        ...
        indentUnit.of("    ".padStart(tab, " ")),   // soft tabs: 4 spaces
        this.modeCompartment.of(opts.mode === "python" ? python() : cpp()),
        baseTheme,
